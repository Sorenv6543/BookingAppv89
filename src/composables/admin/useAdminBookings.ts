  ...
  const createBooking = async (bookingData: BookingFormData): Promise<void> => {
    try {
      ...
      const bookingWithId: Booking = {
        ...
        checkout_time: bookingData.checkout_time as string,
        checkin_time: bookingData.checkin_time as string,
        guest_count: bookingData.guest_count as number | undefined,
        notes: bookingData.notes as string | undefined,
        status: bookingData.status as BookingStatus,
        booking_type: bookingData.booking_type as BookingType,
        priority: (bookingData.priority || 'normal') as 'low' | 'normal' | 'high' | 'urgent',
        assigned_cleaner_id: bookingData.assigned_cleaner_id as string | undefined,
        created_at: new Date().toISOString(),
        updated_at: new Date().toISOString()
      };
