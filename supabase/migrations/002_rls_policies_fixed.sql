-- =====================================================
-- Property Cleaning Scheduler - Row Level Security Policies (FIXED)
-- Migration: 002_rls_policies_fixed.sql
-- Purpose: Enforce multi-tenant security at database level
-- Fixed: Function creation order and dependencies
-- =====================================================
...
-- Create all functions before using them in policies
-- =====================================================
...
-- Commit the functions before proceeding
COMMIT;
BEGIN;
...
-- Now all functions exist and can be referenced
-- =====================================================
...
USING (
  ...
  (private.is_owner() AND owner_id = auth.uid())
  OR
...
-- Note: Basic indexes are already created in the schema migration
-- RLS policies will use the existing indexes:
-- - idx_user_profiles_id (PRIMARY KEY)
-- - idx_user_profiles_role 
-- - idx_properties_owner_id
-- - idx_bookings_owner_id, idx_bookings_property_id

-- Additional role-specific indexes for RLS performance
...
COMMENT ON FUNCTION private.user_owns_property(UUID) IS 'Check if current user owns specified property - used for booking validation';
