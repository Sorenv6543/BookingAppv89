...
-- Migration: 002_rls_policies_simple.sql
...
USING (
  ...
  (
    ...
    AND EXISTS (
      SELECT 1 FROM public.user_profiles up 
      WHERE up.id = auth.uid() AND up.role = 'cleaner'
    )
  )
);
...
WITH CHECK (
  ...
  OR
  -- Cleaner can only update certain fields
  (
    assigned_cleaner_id = auth.uid()
    AND EXISTS (
      SELECT 1 FROM public.user_profiles up 
...
CREATE INDEX IF NOT EXISTS idx_user_profiles_role_cleaner 
ON public.user_profiles(role) WHERE role = 'cleaner';
