...
export default defineConfig({
  ...
  plugins: [
    vue({
      script: {
        defineModel: true, // Enable defineModel macro for v-model bindings
      },
      template: {
        compilerOptions: {
      ...
      features: {
        ...
        optionsAPI: true,               // Keep Options API support
        prodDevtools: true,             // Enable DevTools in production builds
        prodHydrationMismatchDetails: true, // Detailed SSR hydration errors
      }
    }),
    vuetify({ 
  ...
  define: {
    ...
    // Vue DevTools feature flags - essential for debugging
    __VUE_OPTIONS_API__: JSON.stringify(true), // Enable Options API support
    __VUE_PROD_DEVTOOLS__: JSON.stringify(true), // Enable DevTools in production builds
    __VUE_PROD_HYDRATION_MISMATCH_DETAILS__: JSON.stringify(true), // Show hydration mismatch details
  ...
  server: {
    port: 3000,
    open: true,
    host: true, // Allow access from local network for mobile debugging
    sourcemapIgnoreList: false, // Include all source maps for debugging
    // HMR configuration for development
    hmr: {
      overlay: true // Enable HMR error overlay for debugging
    },
    // Native file watching for better performance
    watch: {
